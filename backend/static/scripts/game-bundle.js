(() => {
  ...
  (s.n = (t) => {
    ...
    (() => {
      ...
      var mt = s(123),
        bt = s.n(mt);
      const vt = gt({ query: { roomID: bt()(document.location.pathname) } });
      vt.on("connect", () => {
      ...
      var _t = s(606),
        wt = s.n(_t);
      const Et = bt()(document.location.pathname),
        At = document.querySelector("#messages");
      document
      ...
      const Ct = bt()(document.location.pathname),
        Tt = document.querySelector("#user").dataset.userId,
        Ot = JSON.parse(
          document.querySelector("#boardTiles").dataset.boardTiles
        ),
        Rt = document.getElementsByClassName("player-tile"),
        St = document.getElementById("gameButtons"),
        xt = St.querySelectorAll("button");
      function Bt(t, e, s, n) {
        const i = document.querySelector(`.board-square[x="${t}"][y="${e}"]`),
          r = document.createElement("div");
        (r.id = s),
          (r.classList += "square no-drop"),
          ...
          ({ username: t, message: e, timestamp: s }) => {
            At.appendChild(
              (function (t, e, s) {
                ...
                return (o.innerText = s), n.append(i, r, o), n;
              })(t, e, s)
            );
          }
        );
            ...
            async () => {
              ...
              s.forEach((t) => {
                const e = parseInt(t.getAttribute("x")),
                  ...
                  i = t.querySelector(".player-tile"),
                  r = parseInt(i.getAttribute("id"));
                n.push({
                  boardX: e,
                  boardY: s,
                  canonicalTileID: r,
      ...
      for (let t = 0; t < Ot.length; t++) {
        const e = Ot[t];
        Bt(e.x, e.y, e.tile_id, e.letter);
      }
      kt.on("board-updated", (t) => {
        for (let e = 0; e < t.length; e++) {
          const s = t[e];
          Bt(s.boardX, s.boardY, s.canonicalTileID, s.letter);
        }
      }),
        kt.on("current-player", (t) => {
          if (parseInt(t.user_id) == Tt) {
            xt.forEach((t) => {
              t.disabled = !1;
            });
            for (let t = 0; t < Rt.length; t++)
              Rt[t].setAttribute("draggable", !0);
          } else {
            xt.forEach((t) => {
              t.disabled = !0;
            });
            for (let t = 0; t < Rt.length; t++)
              Rt[t].setAttribute("draggable", !1);
          }
        });
    })();
})();
